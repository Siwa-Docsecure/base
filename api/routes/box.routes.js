...
const storage = multer.diskStorage({
  destination: async (req, file, cb) => {
    ...
    if (req.method === 'POST') {
      ...
      const [rows] = await db.query('SELECT client_code FROM clients WHERE client_id = ?', [clientId]);
      clientCode = rows[0]?.client_code;
    } else if (req.method === 'PUT') {
      const boxId = req.params.boxId;
      const [rows] = await db.query(
        'SELECT c.client_code FROM boxes b JOIN clients c ON b.client_id = c.client_id WHERE b.box_id = ?',
        [boxId]
      );
      clientCode = rows[0]?.client_code;
    }
    if (!clientCode) clientCode = 'unknown';
    
    const uploadDir = path.join(__dirname, '../uploads', clientCode);
  ...
  async (req, res, next) => {
    try {
      const { 
        ...
        search, 
        page = 1, 
        limit = 50, //must make this also adjustable by user
        sortBy = 'created_at',
        sortOrder = 'DESC'
      } = req.query;
 ...
 * @desc    Create new box with optional image upload
 * @access  Admin, Staff (with permission)
 */
router.post('/',
  authorizeRoles('admin', 'staff'),
  authorizePermission('canCreateBoxes'),
  upload.single('boxImage'), // handle file upload
  async (req, res, next) => {
    try {
      const { 
        clientId, 
      ...
      // Optional: validate boxSize against ENUM
      ...
      const boxNumber = formattedBoxIndex; // using only index as box number
      ...
      if (rackingLabelId) {
        ...
        if (labels.length === 0) throw new NotFoundError('Racking label not found');
        if (!labels[0].is_available) throw new ValidationError('Racking label is not available');
      }
      ...
      // Process uploaded image: store relative path
      let boxImage = null;
      if (req.file) {
        const clientCode = clients[0].client_code;
        boxImage = `uploads/${clientCode}/${req.file.filename}`;
      }
      ...
      `, [
        ...
        boxSize || null, dataYears || null, dateRange || null, boxImage
      ]);
      ...
      // If file was uploaded but insertion failed, delete it
 ...
 * @desc    Update box details, optionally replace image
 * @access  Admin, Staff (with permission)
 */
router.put('/:boxId',
  authorizeRoles('admin', 'staff'),
  ...
  async (req, res, next) => {
    try {
      ...
      const { 
        ...
        dataYears,
        dateRange
        // boxImage is not sent as field; it's the file
      } = req.body;
      
      // Check box exists and get current data
      ...
      if (req.file) {
        ...
        if (oldBox.box_image) {
          const oldPath = path.join(__dirname, '..', oldBox.box_image);
        ...
        if (clientCode) {
          newImagePath = `uploads/${clientCode}/${req.file.filename}`;
      ...
      // Execute update
      await db.query(`UPDATE boxes SET ${updates.join(', ')} WHERE box_id = ?`, params);
      ...
      await createAuditLog(
        ...
        { boxDescription, rackingLabelId, retentionYears, boxSize, dataYears, dateRange, boxImage: newImagePath },
        req.ip,
        req.get('user-agent')
      );
      ...
      res.status(200).json({
        ...
        data: { boxId: parseInt(boxId) }
      });
      
    } catch (error) {
      // If a new file was uploaded but update fails, delete it
      if (req.file && fs.existsSync(req.file.path)) {
        fs.unlinkSync(req.file.path);
      }
      next(error);
...
router.post('/bulk/create',
  ...
  async (req, res, next) => {
    try {
      ...
      // ===== 3. Check existing box numbers in database =====
      if (proposedBoxNumbers.length > 0) {
        // Build dynamic placeholders for the IN clause
        const boxPlaceholders = proposedBoxNumbers.map(() => '?').join(',');
        const [existingBoxes] = await db.query(
          `SELECT box_number FROM boxes WHERE box_number IN (${boxPlaceholders})`,
          proposedBoxNumbers
        );
